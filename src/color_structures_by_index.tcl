...
# Fonction pour appliquer la couleur par index
proc apply_colors_by_index {molid colors} {
    # Sélectionne tous les résidus (atomes CA)
    set sel_all [atomselect $molid "name CA"]
    set resid_list [$sel_all get resid]
    set resid_unique [lsort -unique $resid_list]

    # Applique les couleurs dans l'ordre des index
    for {set i 0} {$i < [llength $colors] && $i < [llength $resid_unique]} {incr i} {
    ...
    # Configure la représentation
    mol modcolor 0 $molid ColorID
    mol modstyle 0 $molid NewCartoon
}

# Applique aux deux molécules
apply_colors_by_index 0 $colors
apply_colors_by_index 1 $colors

# Ajuste la vue pour mieux voir
display resetview
scale to 0.8

    ...
    for i, res in enumerate(residues):
        ...
        if 50 <= res_number <= 79:
            if start_idx is None:
                start_idx = i - 0.5  # Start at left edge of first highlighted residue
            end_idx = i + 0.5  # Continuously update to right edge of last highlighted residue

    # Draw a single continuous highlight if any residues were found
    if start_idx is not None and end_idx is not None:
        plt.axvspan(start_idx, end_idx, color='green', alpha=0.3, zorder=0)

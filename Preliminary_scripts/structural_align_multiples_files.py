...
def main(pdb_directory, monomer_number):
    ...
    # Highlight residues between 50 and 79 as a continuous block
    start_idx1 = None
    end_idx1 = None
    # Find the first and last residue indices in the range 50-79
    for i, res in enumerate(residues):
        res_number = int(res.split()[-1])
        if 50 <= res_number <= 79:
            if start_idx1 is None:
                start_idx1 = i - 0.5  # Start at left edge of first highlighted residue
            end_idx1 = i + 0.5  # Continuously update to right edge of last highlighted residue

    # Draw a single continuous highlight if any residues were found
    if start_idx1 is not None and end_idx1 is not None:
        plt.axvspan(start_idx1, end_idx1, color='green', alpha=0.3, zorder=0)

    # Highlight residues between 178 and 190 as a continuous block
    start_idx2 = None
    end_idx2 = None
    # Find the first and last residue indices in the range 178-190
    for i, res in enumerate(residues):
        res_number = int(res.split()[-1])
        if 178 <= res_number <= 190:
            if start_idx2 is None:
                start_idx2 = i - 0.5  # Start at left edge of first highlighted residue
            end_idx2 = i + 0.5  # Continuously update to right edge of last highlighted residue

    # Draw a single continuous highlight if any residues were found
    if start_idx2 is not None and end_idx2 is not None:
        plt.axvspan(start_idx2, end_idx2, color='gray', alpha=0.3, zorder=0)
